...
TEST_CASE("Simple derivatives vertical")
{
    ...
    NodalField<N1,N2,N3> f1(GridType::Staggered);
    f1.SetValue([](stratifloat z){return exp(-(z-0.5)*(z-0.5));}, L);

    // convert to modal for differentiation
    NodalField<N1,N2,N3> f2(GridType::Staggered);
    f2 = Dim3MatMul<Map<const Array<stratifloat, -1, 1>, Aligned16>,stratifloat,stratifloat,N1,N2,N3>(VerticalSecondDerivativeMatrix(L, N3, GridType::Staggered), f1);

    NodalField<N1,N2,N3> expected(GridType::Staggered);
...
TEST_CASE("Dim 1 fourier derivatives")
{
    ...
    ModalField<N1,N2,N3> f2(GridType::Staggered);
    f1.ToModal(f2, false);

    ModalField<N1,N2,N3> f3(GridType::Staggered);
    f3 = Dim1MatMul<Map<const Array<complex, -1, 1>, Aligned16>,complex,complex,N1/2+1,N2,N3>(FourierDerivativeMatrix(L, N1, 1), f2);

    NodalField<N1,N2,N3> f4(GridType::Staggered);
    f3.ToNodal(f4);
    ...
    ModalField<N1,N2,N3> f5(GridType::Staggered);
    ...
    NodalField<N1,N2,N3> f6(GridType::Staggered);
    f5.ToNodal(f6);

    NodalField<N1,N2,N3> expected2(GridType::Staggered);
